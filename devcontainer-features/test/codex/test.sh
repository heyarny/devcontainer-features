...
# Installed-feature contract: the CLI and support files exist, and container
# startup does not run a blocking one-shot config sync.
codex --version
...
! grep -Eq 'sync-config\.sh.*--once' /usr/local/share/codex/entrypoint.sh
...
# The standalone CLI must also run as the standard feature users when present.
...
entrypoint_sync_script="/usr/local/share/codex/sync-config.sh"
...
# The release policy accepts the minimum, newer versions, and supported aliases,
# but rejects older releases and prereleases below the stable minimum.
for supported_release in \
    ...
    0.147.0-alpha.1
do
    /usr/local/share/codex/version-policy.sh "${supported_release}" ||
        fail "Expected ${supported_release} to satisfy the Codex release policy."
done

for unsupported_release in 0.146.0 0.146.1-beta.1
do
    policy_log="${tmp_dir}/policy-${unsupported_release}.log"
    if /usr/local/share/codex/version-policy.sh "${unsupported_release}" > "${policy_log}" 2>&1; then
        fail "Expected ${unsupported_release} to violate the Codex release policy."
    fi
    grep -Fq 'Minimum supported release is 0.146.1' "${policy_log}" ||
        fail "Expected a minimum-release error for ${unsupported_release}."
done

# The updater must enforce the same release floor as the policy helper.
update_policy_log="${tmp_dir}/update-policy.log"
if /usr/local/share/codex/update.sh --release 0.146.0 > "${update_policy_log}" 2>&1; then
    fail "Expected update.sh to reject Codex 0.146.0."
fi
grep -Fq 'Minimum supported release is 0.146.1' "${update_policy_log}" ||
    fail 'Expected update.sh to report the minimum supported release.'
...
# A normal container user can reclaim its conventional .codex home when image
# setup left that directory owned by root.
if [ "$(id -u)" = "0" ] && id vscode >/dev/null 2>&1 && command -v sudo >/dev/null 2>&1; then
...
# Explicit mappings create both links and mirror existing session directories
...
CODEX_HOME="${disabled_config_home}" \
HOME="${tmp_dir}/disabled-config-home" \
    /usr/local/share/codex/sync-config.sh --watch > "${disabled_ready_log}" 2>&1
...
if CODEX_CONFIG_SYNC_SOURCE="${live_symlink_source}" \
    ...
    /usr/local/share/codex/sync-config.sh --once > "${live_symlink_log}" 2>&1; then
...
CODEX_HOME="${missing_source_home}" \
HOME="${tmp_dir}/missing-source-home" \
    /usr/local/share/codex/sync-config.sh --watch > "${missing_source_log}" 2>&1 &
watcher_pid=$!
...
HOME="${tmp_dir}/known-source-home" \
    /usr/local/share/codex/sync-config.sh --watch > "${known_source_log}" 2>&1 &
watcher_pid=$!
...
start_race_watcher() {
    ...
    HOME="${tmp_dir}/race-home" \
        /usr/local/share/codex/sync-config.sh --watch > "${race_log}" 2>&1 &
    watcher_pid=$!
...
HOME="${tmp_dir}/initial-conflict-home" \
    /usr/local/share/codex/sync-config.sh --once > "${initial_conflict_log}" 2>&1
...
HOME="${tmp_dir}/runtime-home" \
    /usr/local/share/codex/sync-config.sh --watch > "${runtime_log}" 2>&1 &
watcher_pid=$!
...
CODEX_ENTRYPOINT_TEST_SESSION_PID_FILE="${entrypoint_session_pid_file}" \
    setsid sh -c 'printf "%s\n" "$$" > "${CODEX_ENTRYPOINT_TEST_SESSION_PID_FILE}"; exec "$@"' sh \
        /usr/local/share/codex/entrypoint.sh "${entrypoint_payload}" > "${entrypoint_output}" 2>&1 &
entrypoint_launcher_pid=$!
...
CODEX_ENTRYPOINT_TEST_SESSION_PID_FILE="${entrypoint_session_pid_file}" \
    setsid sh -c 'printf "%s\n" "$$" > "${CODEX_ENTRYPOINT_TEST_SESSION_PID_FILE}"; exec "$@"' sh \
        /usr/local/share/codex/entrypoint.sh "${timeout_payload}" > "${timeout_output}" 2>&1 &
entrypoint_launcher_pid=$!
